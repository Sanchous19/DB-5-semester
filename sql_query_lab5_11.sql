USE mydb;
SELECT name1.name AS 'фамилия', name2.name AS 'имя', name3.name AS 'отчество', COUNT(DISTINCT ratingtypeShort) AS 'количество уникальных значений рейтинга', AVG(ratingtypeShort) AS 'средний рейтинг', MIN(ratingtypeShort) AS 'минимальный рейтинг', MAX(ratingtypeShort) AS 'максимальный рейтинг', COUNT(ratingtypeShort) AS 'количество оценок', DATE_FORMAT(DatePayed, '%d %m %Y') AS 'дата начала сдачи предмета', DATE_FORMAT(lessons.Date, '%d %m %Y') AS 'дата окончания сдачи предмета', idCurs AS 'номер предмета'
FROM lessons
...
GROUP BY idStudent, idCurs, DATE_FORMAT(lessons.Date, '%d %m %Y');